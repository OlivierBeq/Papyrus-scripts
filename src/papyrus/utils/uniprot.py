...
def uniprot_information(accession: Union[str, List[str]]) -> pd.DataFrame:
    """Obtain protein information from UniProt.

    :param accession: UniProt accessions of proteins to be obtained
    """
    if isinstance(accession, str):
        accession = [accession]
    data = []
    # Only 25 accesisons can be processed at the same time
    for i in range(0, len(accession), 25):
        response = pd.read_csqv('https://rest.uniprot.org/uniprotkb/search?query=accession:' +
                                '+OR+'.join(accession[i: i+25]) +
                                '&fields=accession,id,reviewed,protein_name,'
                                'gene_names,organism_name,length,sequence&format=tsv',
                                sep='\t')
        data.append(response)
    data = pd.concat(data)
    data.rename(columns={'Entry': 'Accession',
