...
def _build_filter_method(spec: _FilterSpec) -> Callable:
    ...
    def method(self, *args, **kwargs):
        ...
        if spec.pre_hook is not None:
            spec.pre_hook(self)
        call_kwargs = {'data': self.papyrus_bioactivity_data}
        call_kwargs.update({tname: getattr(self, attr) for tname, attr in spec.context.items()})
        ...
        return self._wrap(getattr(preprocess, spec.target_name)(**call_kwargs))
